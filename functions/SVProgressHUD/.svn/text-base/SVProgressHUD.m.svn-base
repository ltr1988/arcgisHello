...
static UIImage *SVProgressHUDBackgroundImage;
...
+ (void)setBackgroundImage:(UIImage*)bgImage{
    ...
    [self sharedView].hudView.image = bgImage;
    SVProgressHUDBackgroundImage = bgImage;
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        SVProgressHUDBackgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.8];
        SVProgressHUDForegroundColor = [UIColor whiteColor];
        SVProgressHUDBackgroundImage = [[UIImage imageNamed:@"SVProgressHUD.bundle/toast"] imageWithRenderingMode:UIImageRenderingModeAutomatic];
...
- (void)updatePosition {
    ...
    if(string) {
        ...
        //rdm平台上报了很多此方法在7.0上crash，怀疑此方法在7.0下支持不好，因此加强判断8.0才用此方法
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0f )
        {
          stringRect = [string boundingRectWithSize:constraintSize
            ...
            //怀疑 sizeWithAttributes 在iOS7上支持不好，先用 deprecated 的 sizeWithFont 观察crash情况
...
//#pragma clang diagnostic ignored "-Wdeprecated"
                ...
                //stringSize = [string sizeWithFont:font constrainedToSize:CGSizeMake(200.0f, 300.0f)];
//#pragma clang diagnostic pop
//            }
        
            //sizeWithFont: ConstrainedToSize:方法在iOS7上已经deprecated，还是会引起crash，下面是替换的solution
            self.stringLabel.font = font;
            stringSize = [self.stringLabel sizeThatFits:CGSizeMake(200.0f, 300.0f)];
...
- (UIImageView *)hudView {
    if(!_hudView) {
        _hudView = [[UIImageView alloc] initWithFrame:CGRectZero];
