...
-(NSArray *)defaultUIStyleMapping
{
    return @[
             ...
             @{
                 @"group":@"外观检查",
                 @"outwardwhole":@[@(SheetUIStyle_Switch),@(11)],
                 @"outwardserious":@[@(SheetUIStyle_Switch),@(12)],
                 @"outwardleakage":@[@(SheetUIStyle_Switch),@(13)],
                 },
             
             @{
                 @"group":@"紧固件检查",
                 
                 @"fastenersolid":@[@(SheetUIStyle_Switch),@(14)],
                 },
             
             @{
                 @"group":@"设备运行状态检查",
                 
                 @"devicerun":@[@(SheetUIStyle_Switch),@(15)],
                 @"devicedisplay":@[@(SheetUIStyle_Switch),@(16)],
                 @"devicepressure":@[@(SheetUIStyle_Switch),@(17)],
                 @"deviceshort":@[@(SheetUIStyle_Switch),@(18)],
                 },
        
             @{
                 @"group":@"",
                 @"remark":@[@(SheetUIStyle_Text),@(19)],
                 @"solution":@[@(SheetUIStyle_Text),@(20)],
                 },
             ];
...
-(NSDictionary *)defaultUITextMapping
{
    return @{
             ...
             @"outwardwhole":@"外壳无破损",
             @"outwardserious":@"无明显油渍、污渍；无明显锈蚀",
             @"outwardleakage":@"止水完好，渗漏量正常",
             @"fastenersolid":@"螺栓无松动、脱落",
