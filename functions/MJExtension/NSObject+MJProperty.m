...
+ (NSMutableArray *)properties
{
    ...
    if (cachedProperties == nil) {
        // Check if the properties of class has cached before process launching
        // to avoid multiple threads competing for this resource and crashing app.
        //
#ifdef DEBUG
        if (NO == (NSThread.isMainThread && (nil == UIApplication.sharedApplication))) {
            NSLog(@"Please register class '%@' in QRModelsManager", NSStringFromClass(self));
            abort();
        }
#endif
