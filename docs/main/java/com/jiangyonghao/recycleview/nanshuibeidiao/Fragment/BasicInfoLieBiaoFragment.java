 ...
 */
public class BasicInfoLieBiaoFragment extends Fragment {
    private ListView listview_fragment_basic_liebiao;
    private BasicInfoFragmentItemAdapter basicInfoFragmentItemAdapter = null;

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.fragment_basic_liebiao, container, false);
        listview_fragment_basic_liebiao = (ListView) v.findViewById(R.id.listview_fragment_basic_liebiao);
        ArrayList<SheBeiInfo> list = new ArrayList<>();
        for (int i = 0; i < 30; i++) {
            list.add(new SheBeiInfo("电动蝶阀", "DN1200", "1"));
        }
        basicInfoFragmentItemAdapter = new BasicInfoFragmentItemAdapter(getActivity());
        basicInfoFragmentItemAdapter.addDatas(list);
        listview_fragment_basic_liebiao.setAdapter(basicInfoFragmentItemAdapter);
        return v;
    }
